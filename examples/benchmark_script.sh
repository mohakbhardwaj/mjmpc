...
run_files=(0 1) #idxs of config files to run
run_controllers=(0 1) #idxs of controllers to run
...
for ((j=0;j<${#run_files[@]};++j)); do
	...
	for ((i=0;i<${#run_controllers[@]};++i)); do
      ...
      python job_script.py --config_file configs/${config_files[config_idx]} --controller_type ${controllers[controller_idx]}
	done
done
